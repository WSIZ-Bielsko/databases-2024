...
Create a aiohttp application with paths corresponding to all
CRUD operations from the class DogsCRUD:
...
class DogsCRUD:
    ...
    async def create_dog(self, dog: Dog) -> Dog:
        async with self.pool.acquire() as connection:
            query = "INSERT INTO dogs (id, breed_id, lineage,
            birthdate, name) VALUES ($1, $2, $3, $4, $5) RETURNING *"
            record = await connection.fetchrow(query, dog.id,
            dog.breed_id, dog.lineage, dog.birthdate, dog.name)
            return Dog(**record)
    ...
    async def update_dog(self, dog_id: uuid.UUID, dog: Dog) -> None:
        async with self.pool.acquire() as connection:
            query = "UPDATE dogs SET breed_id = $1, lineage = $2,
...
async def app_factory():
    """
    Function run at the startup of the application. If some
    async initialization is needed - put it in here.
